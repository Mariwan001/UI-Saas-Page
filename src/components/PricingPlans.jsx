         ...
         <div className="max-w-7xl mx-auto px-4 mt-20">
            ...
            variants={{
                ...
                {PLANS_CONTENT.plans.map((plan, index) => (
                    <motion.div key={index} custom={index} variants={containerVariants} className={`p-8 rounded-xl shadow-lg bg-neutral-950 ${plan.popular ? "border border-blue-900/80" : "border border-neutral-800"}`}>
                        {plan.popular && (
                            <div className="text-center mb-4">
                                <span className="bg-blue-600 text-white text-xs py-1 px-3 rounded-full uppercase">{PLANS_CONTENT.popularBadge}</span>
                            </div>
                        )}
                        <h3 className="text-lg lg:text-xl tracking-tighter uppercase">{plan.name}</h3>
